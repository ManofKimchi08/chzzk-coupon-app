...
- **🎙️ 진행자 전용 관리자 대시보드 (`/admin`)**:
  - 쿠폰 코드 및 당첨자 명단 대량 CSV 업로드
  - 실시간 수령 내역 검색, 수령 상태 초기화(리셋) 및 CSV 내보내기
  - 시청자 화면 공지 문구 실시간 편집
...
│   ├── .env                         # ⚙️ API 키 및 비밀번호 설정 파일 (메모장으로 수정)
...
## 🔑 최초 1회 필수 환경설정 (`.env` 세팅법)

인수인계받은 담당자는 서버를 켜기 전, **`.env`** 파일에 치지직 API 키와 관리자 정보를 세팅해야 합니다.
...
3. **사용 API**: `치지직 (CHZZK)` 선택 ➔ **사용자 프로필/채널 ID 조회** 권한 체크
...
### 2단계: `.env` 파일 정보 작성
`ChzzkCouponServer` 폴더 내의 `.env` 파일을 메모장으로 열어 아래 값을 입력하고 저장합니다.
